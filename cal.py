...
with st.sidebar:
    ...
    st.download_button(label=f"⬇️ {half} PDF 저장", data=pdf_data, file_name=f"2026_calendar_{half}.pdf", mime="application/pdf")

# 4. 메인 달력 출력 (2열 배치)
cols = st.columns(2)
for m in range(1, 13):
    with cols[(m-1)%2]:
        st.markdown(f"**{m}월**") # 월 표시도 굵게만 처리
        ...
        for week in cal:
            ...
            for day in week:
                if day == 0:
                    ...
                    html += f"""
                    <td style='background-color: {color}; color: white; opacity: {opacity}; border: {border}; padding: 4px; height: 38px;'>
